...
public class Day07PartTwo {
    ...
    public class Hand implements Comparable<Hand> {
        ...
        public Hand(String cards, int bet) {
            ...
            Integer numberOfJokers = countByCard.getOrDefault('J', 0);
            //highest other card than Joker
            Optional<Character> first = countByCard.entrySet().stream()
                    .filter(characterIntegerEntry -> characterIntegerEntry.getKey() != 'J')
                    .sorted((o1, o2) -> Integer.compare(o2.getValue(), o1.getValue()))
                    .map(Map.Entry::getKey)
                    .findFirst();
            if (first.isPresent()) {
                countByCard.put(first.get(), countByCard.get(first.get()) + numberOfJokers);
                //remove jokers as we used them as a different card type T JJ does not become FULL HOUSE but THREE OF A KIND
                countByCard.remove('J');
            }
